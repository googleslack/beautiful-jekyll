...
<html lang="en">
<head>
    <title>Code coverage report for lib/client/bespoke-client.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../../index.html">all files</a> / <a href="index.html">lib/client/</a> bespoke-client.js
    </h1>
    <div class='clearfix'>
    ...
    onWebhookReceived(request) {
        ...
        tcpClient.transmit("localhost", self.targetPort, request.toTCP(), function (data, error, message) {
            if (data != null) {
                ...
                <span class="missing-if-branch" title="else path not taken" >E</span>if (httpBuffer.complete()) {
                    logging_helper_1.LoggingHelper.info(Logger, "ResponseReceived ID: " + request.id());
                    let payload = null;
                    <span class="missing-if-branch" title="else path not taken" >E</span>if (httpBuffer.isJSON()) {
                        payload = string_util_1.StringUtil.prettyPrintJSON(httpBuffer.body().toString());
                    }
                    else {
<span class="cstat-no" title="statement not covered" >                        payload = httpBuffer.body().toString();</span>
                    }
                    logging_helper_1.LoggingHelper.verbose(Logger, "Payload:\n" + payload);
                    self.socketHandler.send(httpBuffer.raw().toString(), request.id());
                }
            }
            else <span class="missing-if-branch" title="else path not taken" >E</span>if (error !== null &amp;&amp; error !== undefined) {
                <span class="missing-if-branch" title="else path not taken" >E</span>if (error === global_2.NetworkErrorType.CONNECTION_REFUSED) {
                    logging_helper_1.LoggingHelper.error(Logger, "CLIENT Connection Refused, Port " + self.targetPort + ". Is your server running?");
                }
                <span class="missing-if-branch" title="else path not taken" >E</span>if (self.onError != null) {
    ...
    connected(error) {
        if (error !== undefined &amp;&amp; error !== null) {
            ...
            this.shutdown();
            <span class="missing-if-branch" title="else path not taken" >E</span>if (this.onConnect !== undefined &amp;&amp; this.onConnect !== null) {
...
//# sourceMappingURL=bespoke-client.js.map</pre></td></tr>
</table></pre>
